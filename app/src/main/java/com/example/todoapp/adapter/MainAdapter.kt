...
class MainAdapter(val todoItemList: List<TodoItem>, private val onClickListener: OnClickListener) :
    ...
    override fun onBindViewHolder(holder: MainViewHolder, position: Int) {
       ...
       if (todoItemList[position].flag == true) {
        ...
        if(todoItemList[position].importance == Importance.URGENT && !holder.checkBoxItem.isChecked) {
            holder.checkBoxItem.buttonTintList = ColorStateList(
                arrayOf(intArrayOf(android.R.attr.state_checkable), intArrayOf()),
